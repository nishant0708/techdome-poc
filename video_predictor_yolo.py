...
### Process a Frame and making an annotation for a person
...
# Filter results for the "person" class (class index for person is 0 in COCO dataset)
persons = [det for det in results[0].boxes if det.cls == 0]

# Iterate through detected persons and process
for idx, person in enumerate(persons):
    # Extract bounding box coordinates
    x_min, y_min, x_max, y_max = person.xyxy[0].tolist()
    box = np.array([x_min, y_min, x_max, y_max], dtype=np.float32)

    # Assign a unique object ID
    ann_obj_id = idx + 1

    # Add the box to your interaction model
    ...
    # Display results for the current person
    plt.figure(figsize=(9, 6))
    plt.title(f"Frame {ann_frame_idx} - Person {ann_obj_id}")
    plt.imshow(frame)
    show_box(box, plt.gca())

    # Ensure the mask for the current object ID is displayed
    if out_mask_logits is not None and len(out_mask_logits) > idx:
        show_mask((out_mask_logits[idx] > 0.0).cpu().numpy(), plt.gca(), obj_id=out_obj_ids[idx])
...
### Creating Video form the Segmentation Results
